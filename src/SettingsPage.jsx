...
import { Bell, Building2, Database, Save, Trash2, Upload, UserPlus } from 'lucide-react';
...
export default function SettingsManager({ user, company, companies, setCompanies, companyId, refresh }) {
  ...
  function show(text) { setMessage(text); setTimeout(() => setMessage(''), 4000); }

  async function saveCompany() {
    const data = await api('/api/companies', { method: 'PATCH', body: JSON.stringify({ id: company.id, name: companyName, isActive: true }) });
    setCompanies(companies.map((c) => c.id === data.company.id ? data.company : c));
    show('Company saved.');
  }
  async function addCompany() {
    const data = await api('/api/companies', { method: 'POST', body: JSON.stringify({ name: newCompany }) });
  ...
  async function addUser() {
    ...
    setNewUser({ username: '', password: '', displayName: '', role: 'user', companyId: companyId || '' });
    show('User added.');
  }
  async function saveUser(row, patch) {
    const data = await api('/api/users', { method: 'PATCH', body: JSON.stringify({ ...row, ...patch }) });
    setUsers(users.map((u) => u.id === data.user.id ? data.user : u));
    show('User saved.');
  }
  async function deleteUser(row) {
    if (!confirm(`Delete ${row.username}?`)) return;
    await api(`/api/users?id=${row.id}`, { method: 'DELETE' });
    setUsers(users.filter((u) => u.id !== row.id));
    show('User deleted.');
  }
  async function addEmail() {
    const data = await api('/api/notification-emails', { method: 'POST', body: JSON.stringify(newEmail) });
    setEmails([...emails, data.email]);
    setNewEmail({ label: '', email: '' });
    show('Email added.');
  }
  async function saveEmail(row, patch) {
    const data = await api('/api/notification-emails', { method: 'PATCH', body: JSON.stringify({ ...row, ...patch }) });
    setEmails(emails.map((e) => e.id === data.email.id ? data.email : e));
    show('Email saved.');
  }
  async function deleteEmail(row) {
    if (!confirm(`Delete ${row.email}?`)) return;
    await api(`/api/notification-emails?id=${row.id}`, { method: 'DELETE' });
    setEmails(emails.filter((e) => e.id !== row.id));
    show('Email deleted.');
  }
  async function importRows() {
    const rows = parseCsv(csv);
    const data = await api('/api/import-applicants', { method: 'POST', body: JSON.stringify({ companyId, rows }) });
    setCsv('');
    await refresh?.();
    show(`Imported ${data.imported}. Skipped ${data.skipped}.`);
  }

  return <>
    <div className="page-header"><div><h1>Settings</h1><p>Users, companies, notification emails, and imports</p></div><button className="secondary-btn" onClick={loadSettings}>Refresh</button></div>
