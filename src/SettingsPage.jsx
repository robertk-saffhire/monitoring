...
export default function SettingsManager({ user, company, companies, setCompanies, companyId, refresh }) {
  ...
  async function importRows() { try { const rows = parseCsv(csv); const data = await api('/api/import-applicants', { method: 'POST', body: JSON.stringify({ companyId, rows }) }); setCsv(''); await refresh?.(); show(`Imported ${data.imported}. Skipped ${data.skipped}.`); } catch (err) { fail(err, 'Could not import CSV.'); } }
  ...
  return <>
    ...
    <section className="card wide-card settings-card"><h2><CheckCircle size={19}/> System Check</h2><p className="muted">Checks database connection and required tables.</p><button className="secondary-btn" disabled={!isAdmin} onClick={runSystemCheck}>Run System Check</button>{checks.length ? <div className="system-check-list">{checks.map((c,i)=><div key={i} className={c.ok ? 'check-ok' : 'check-bad'}><b>{c.ok ? '✓' : '!'}</b><span>{c.name}</span><small>{c.detail}</small></div>)}</div> : null}</section>
