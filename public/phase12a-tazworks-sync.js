(function () {
  ...
  function api(path, options) {
    return fetch('/api/index?path=' + encodeURIComponent(path), Object.assign({
      credentials: 'include'
    }, options || {}, {
  ...
  async function runSync() {
    ...
    try {
      ...
      if (button) {
        ...
        button.textContent = 'Run TazWorks Sync Now';
      }
    }
  }
  ...
  function ensurePanel() {
    ...
    panel.innerHTML = `
      ...
      <div class="phase12a-warning">
        Server-side only. Browser does not call TazWorks or the proxy directly. Client GUID and proxy secret stay in Vercel ENV.
      </div>
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      .phase12a-actions button:hover { background: #dcfce7; }
