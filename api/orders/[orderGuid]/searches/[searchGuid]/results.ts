...
function pathParams(req: any) {
  const orderGuid = req.query?.orderGuid ? String(req.query.orderGuid) : '';
  const searchGuid = req.query?.searchGuid ? String(req.query.searchGuid) : '';
  if (orderGuid && searchGuid) return { orderGuid, searchGuid };

  const url = new URL(req.url || '/', 'https://local.test');
  const match = url.pathname.match(/\/api\/orders\/([^/]+)\/searches\/([^/]+)\/results/i);
  return {
    orderGuid: match ? decodeURIComponent(match[1]) : '',
    searchGuid: match ? decodeURIComponent(match[2]) : '',
  };
}
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const env = getEnv();
    const { orderGuid, searchGuid } = pathParams(req);
    if (!orderGuid || !searchGuid) return json(res, 400, { status: 'error', message: 'Order GUID and Search GUID are required.' });

    const q = getQuery(req);
    const resultType = String(q.get('resultType') || 'EDITOR').trim() || 'EDITOR';
