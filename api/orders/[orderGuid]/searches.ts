...
function pathOrderGuid(req: any) {
  if (req.query?.orderGuid) return String(req.query.orderGuid);
  const url = new URL(req.url || '/', 'https://local.test');
  const match = url.pathname.match(/\/api\/orders\/([^/]+)\/searches/i);
...
export default async function handler(req: any, res: any) {
  try {
    ...
    const env = getEnv();
    const orderGuid = pathOrderGuid(req);
    if (!orderGuid) return json(res, 400, { status: 'error', message: 'Order GUID is required.' });
